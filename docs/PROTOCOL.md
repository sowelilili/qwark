...
Build 27 starts a measured one-second quiet window when IS_INGAME first becomes
true, before any PID, title or memory query. During that second, existing TCP
connections pause receiving, dispatching and replying; telemetry and autosplit
UDP sends pause as well. Queued frames resume in order after the window, with no
extra request buffers. The module does not control the OS's TCP acknowledgements
or traffic generated by other plugins.

Game transitions remain protected from request-buffer allocations. On an existing
connection, HELLO, HEARTBEAT, SUBSCRIBE, UNSUBSCRIBE and GET_STATE with at most two
payload bytes use fixed buffers and continue during BOOTING/QUITTING outside the
first quiet second. Other
requests are drained without allocating and answered BUSY; retry after boot.
New connections are closed before a client thread is created during this window.
An existing connection holding bulk buffers when the transition starts is
cancelled and closed; its owner releases the buffers after any executing command
returns, so that request may receive no reply. Reconnect normally.

Connected sockets use nonblocking I/O. Payload receives and reply sends each have
a two-second deadline, excluding time paused for boot; expiry closes the
connection and releases its buffers.
Waiting for an idle request header holds no page allocation and has no deadline.
